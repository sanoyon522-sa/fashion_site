...
import AllHoodie from '../HoodieComponent/AllHoodie';
import BlackHoodie from '../HoodieComponent/BlackHoodie';
import CreameWhiteHoodie from '../HoodieComponent/CreameWhiteHoodie';
import BlueHoodie from '../HoodieComponent/BlueHoodie';
import OliveHoodie from '../HoodieComponent/OliveHoodie';


const Hoodie = () => {
  const [openFilter, setopenFilter] = useState(false);
  const [color, setcolor] = useState('hoodies');
  const changetoOlive = () => {
    setcolor('olive');
  };
  const changeBlack = () => {
  ...
  return (
    <div className="max-w-[1320px] mx-auto flex flex-col lg:flex-row lg:justify-between gap-x-6 gap-y-4">
        ...
        <div className="bg-white rounded-xl lg:sticky top-15 shadow-2xl border border-gray-300 mb-5">
          <div className="flex flex-row gap-x-4  z-100 pl-5 py-5">
            ...
            <div className="flex flex-col gap-y-4 bg-gray-100 p-7 rounded-lg border border-olive">
              ...
              <button
                onClick={changetoOlive}
                className={`${
                  color === 'olive' ? 'text-olive' : 'text-gray-800'
                } font-bold text-base cursor-pointer`}
              >
                Olive
              </button>
      ...
      {color === 'all' ? <AllHoodie /> : null}
      {color === 'black' ? <BlackHoodie /> : null}
      {color === 'olive' ? <OliveHoodie /> : null}
      {color === 'blue' ? <BlueHoodie /> : null}
      {color === 'white' ? <CreameWhiteHoodie /> : null}
      {color === 'hoodies' ? <AllHoodie /> : null}
    </div>
  );
};
