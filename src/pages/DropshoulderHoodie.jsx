...
import AllDHoodie from '../DropSholderHoodieComponent/AllDHoodie';
import BlackDhoodie from '../DropSholderHoodieComponent/BlackDhoodie';
import AutumnDhoodie from '../DropSholderHoodieComponent/AutumnDhoodie';
import BlueDhoodie from '../DropSholderHoodieComponent/BlueDhoodie';
import GrayDhoodie from '../DropSholderHoodieComponent/GrayDhoodie';

const DropshoulderHoodie = () => {
  const [openFilter, setopenFilter] = useState(false);
  const [color, setcolor] = useState('dhoodies');
  const changetoAutumn = () => {
    setcolor('autumn');
  };
  const changeBlack = () => {
    setcolor('black');
  ...
  return (
    <div className="max-w-[1320px] mx-auto flex flex-col lg:flex-row lg:justify-between gap-x-6 gap-y-4">
        ...
        <div className="bg-white rounded-xl lg:sticky top-15 shadow-2xl border border-gray-300 mb-5">
          <div className="flex flex-row gap-x-4  z-100 pl-5 py-5">
            ...
            <div className="flex flex-col gap-y-4 bg-gray-100 p-7 rounded-lg border border-olive">
              ...
              <button
                onClick={changetoAutumn}
                className={`${
                  color === 'autumn' ? 'text-olive' : 'text-gray-800'
                } font-bold text-base cursor-pointer`}
              >
                Autumn
              </button>
              ...
              <button
                ...
                className={`${
                  color === 'blue' ? 'text-olive' : 'text-gray-800'
                } font-bold text-base cursor-pointer`}
              >
                Brown
              </button>
      ...
      {color === 'all' ? <AllDHoodie /> : null}
      {color === 'black' ? <BlackDhoodie /> : null}
      {color === 'autumn' ? <AutumnDhoodie /> : null}
      {color === 'blue' ? <BlueDhoodie /> : null}
      {color === 'gray' ? <GrayDhoodie /> : null}
      {color === 'dhoodies' ? <AllDHoodie /> : null}
    </div>
  );
};
