...
import AllShoes from '../ShoesComponent/AllShoes';
import BlackShoes from '../ShoesComponent/BlackShoes';
import WhiteShoes from '../ShoesComponent/WhiteShoes';
import BrownShoes from '../ShoesComponent/BrownShoes';



const Shoes = () => {
  const [openFilter, setopenFilter] = useState(false);
  const [color, setcolor] = useState('shoes');
  const changeBlack = () => {
    setcolor('black');
  };
  const changetoBrown = () => {
  ...
  return (
    <div className="max-w-[1320px] mx-auto flex flex-col lg:flex-row lg:justify-between gap-x-6 gap-y-4">
      ...
      {color === 'all' ? <AllShoes /> : null}
      {color === 'black' ? <BlackShoes /> : null}
      {color === 'brown' ? <BrownShoes /> : null}
      {color === 'white' ? <WhiteShoes /> : null}
      {color === 'shoes' ? <AllShoes /> : null}
    </div>
  );
};
